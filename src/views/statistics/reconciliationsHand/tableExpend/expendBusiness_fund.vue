<template>
    <ul v-if="rowTemp.appUnionCheckTransaction && rowTemp.appUnionCheckTransaction.length > 0">
        <li v-for="item in rowTemp.appUnionCheckTransaction">
            <p class="left" v-if="item.fundName">通道:<br/>{{item.fundName}}</p>
            <p class="left" v-else="item.fundName">通道:--</p>
            <p class="left" v-if="item.fundTransactionNo">通道单号:<br/>{{item.fundTransactionNo}}</p>
            <p class="left" v-else="item.fundTransactionNoo">通道单号:--</p>
